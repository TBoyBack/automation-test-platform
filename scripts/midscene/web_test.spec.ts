...
midsceneTest('内容页面 - 草稿编辑演示', async ({ page }) => {
  const agent = new (await import('@midscene/web/playwright')).PlaywrightAgent(page);

  // 1. 打开静态示例页面，避免默认测试触达真实社交账号
  await page.goto('https://www.example.com');
  
  // 2. 验证页面基础内容
  await agent.aiAssert('页面显示 Example Domain 标题');
  
  // 3. 读取页面摘要，保持示例为只读流程
  const pageSummary = await agent.aiQuery(
    '{ title: string, description: string }',
    '页面标题和说明文字'
  );

  console.log('示例页面摘要:', pageSummary);
});
