...
midsceneTest('社交内容 - 创建草稿预览', async ({ page }) => {
  const agent = new (await import('@midscene/web/playwright')).PlaywrightAgent(page);

  // 使用本地页面演示草稿编辑，避免对真实社交平台产生副作用。
  await page.setContent(`
    <main>
      ...
      <button type="button" onclick="
        document.querySelector('[data-testid=draft-preview]').textContent =
          document.querySelector('textarea').value;
        document.querySelector('[role=status]').textContent = '草稿已保存';
      ">保存草稿</button>
      <p role="status"></p>
      <section>
        <h2>草稿预览</h2>
        <p data-testid="draft-preview"></p>
      </section>
    </main>
  `);
  ...
  // 4. 等待保存成功
  await agent.aiWaitFor('草稿已保存的提示');
  
  // 5. 断言草稿预览
  await agent.aiAssert('草稿预览区域显示刚输入的内容');
});
