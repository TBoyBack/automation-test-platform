...
midsceneTest('社交媒体 - 保存图文草稿', async ({ page }) => {
  const agent = new (await import('@midscene/web/playwright')).PlaywrightAgent(page);

  // 1. 使用本地页面演示发帖表单，避免误触真实社交平台
  await page.setContent(`
    <main>
      ...
      <button type="button">保存草稿按钮</button>
      <p role="status">等待编辑</p>
    </main>
    <script>
  ...
  // 6. 断言只保存本地草稿，不发布到外部平台
  await agent.aiAssert('页面显示草稿已保存，未发布到任何外部平台');
});
